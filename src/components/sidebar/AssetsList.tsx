...
export const AssetsList: React.FC<AssetsListProps> = ({ project, setProject }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              // Save to storage adapter (returns ID)
              const assetId = await adapter.saveAsset(file);
              ...
              const newAsset: Asset = {
                  ...
                  name: file.name,
                  type,
                  url: '', // URL is now loaded on demand or ephemeral. We keep it empty in the model.
      ...
      if (newAssets.length > 0) {
          ...
          // Trigger save project to persist the new asset metadata
